...
- **版本**：1.5.0（版本紀錄；下游審查者更名為 `aoccqa-quality-reviewer`、上游明列 `rule-loader`）
...
- Requirement Matrix（含 Requirement ID 與確認狀態），或 `fsd-parser` 的六段需求分析報告。
...
- **不重新解析**原始 FSD/PRD/截圖（屬 Phase A 的 `fsd-parser`）。
...
- 新功能沒有既有測案 baseline —— 這是產「第一版」測案的正確入口（既有測案補強請改用 `scenario-expander`）。
