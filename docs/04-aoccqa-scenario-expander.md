...
- **版本**：frontmatter 未標版本
- **Phase / 產線位置**：Phase B｜補強環節（tc-generator 之後、quality-reviewer 之前）
- **產線**：`fsd-parser` → `tc-generator` → **`scenario-expander`** → `quality-reviewer`
...
當既有或改版功能已有審過/維護中/執行過/前版本的測案、需要更強覆蓋時使用；或用來補強一份已獨立審查過的 `tc-generator` 草稿的缺口。
...
**拒絕/暫停**：同一輪 tc-generator 剛產且未獨立審查的草稿、僅供自我比對的草稿、條件不足以判斷覆蓋的清單、與當前範圍無關的測項。
...
- 產線剛出的 tc-generator 草稿已獨立審查過，想再補強覆蓋深度。
