...
- **原始檔單次讀取**：僅 `aoccqa-fsd-parser`（步驟 1）與 `aoccqa-quality-reviewer`（步驟 6，讀原文查核）碰原文；中間產案流程一律引用 Requirement ID，不重複讀原始檔。
- **唯一人工關卡**：步驟 2「規格確認」由 QA＋PM 執行，未取得結論不得往下。
- **獨立呼叫**：`aoccqa-case-exporter`（步驟 8）任何階段皆可獨立呼叫；`aoccqa-decision-archiver`（步驟 9）與匯出互不依賴。

## 回退路徑（虛線，會反覆執行）

- ① `aoccqa-rule-loader`、② `規格確認`、③ `aoccqa-quality-reviewer` 為圖中標記的回饋節點。
- `aoccqa-quality-reviewer` 產出的差異指令可回退至前段（aoccqa-tc-generator／aoccqa-scenario-expander）反覆修正，直到 QA 判定處置為「接受」。
